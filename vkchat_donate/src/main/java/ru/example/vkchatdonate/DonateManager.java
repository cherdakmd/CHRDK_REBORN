...
public class DonateManager {
    ...
    public void checkExpiredStatus(Player p) {} // stub — реализовано в LuckPerms через temp permissions

    public int getDonorCount() { return (int) statuses.values().stream().filter(s -> s.price > 0).count(); }

    public double getTotalDonatedAll() { return totalDonated.values().stream().mapToDouble(d -> d).sum(); }
